...
_DELTA_T_BUCKETS = ("short_term", "long_term")


def _zero_bucket_dict(names: tuple[str, ...]) -> dict[str, dict[str, float]]:
    return {name: {"loss_sum": 0.0, "count": 0} for name in names}
...
class EvaluationResult:
    ...
    # Per-bucket loss accumulators — sums and counts, not means, so they
    # aggregate cleanly across splits without ever concatenating raw
    # per-review tensors.
...
class EvaluationAggregate:
    ...
    loss_by_rating: dict[str, dict[str, float]] = field(
        default_factory=lambda: _zero_bucket_dict(_RATING_NAMES)
    )
    ...
    def add(self, result: EvaluationResult) -> None:
        ...
        for name in _RATING_NAMES:
            self.loss_by_rating[name]["loss_sum"] += result.loss_by_rating[name]["loss_sum"]
            self.loss_by_rating[name]["count"] += result.loss_by_rating[name]["count"]
...
def train_iter(
    ...
    # iter-75: L2 penalty shrinks toward the FIXED FSRS7_DEFAULT (see
    # fsrs_v7_helpers.penalty_loss). The iter-24 empirical-Bayes anchor (live
    # population mean over all rows) was removed as a joint-optimization exploit.
...
def evaluate_on_test_set(fsrs_params: torch.Tensor, users: list[int], data: Data) -> EvaluationResult:
    ...
    rating_test = data.review_data.rating[data.test_index]
    delta_t_test = data.review_data.elapsed_days_real[data.test_index]
    label = (rating_test > 1).float()
    ...
    # Per-rating loss accumulators (sums + counts so the aggregator can merge
    # cleanly across splits without ever concatenating raw per-review tensors).
    loss_by_rating: dict[str, dict[str, float]] = {}
    for rating_value, name in zip((1, 2, 3, 4), _RATING_NAMES):
        mask = rating_test == rating_value
        loss_by_rating[name] = {
    ...
    loss_by_delta_t: dict[str, dict[str, float]] = {
        "short_term": {
            "loss_sum": float(loss[short_mask].sum().item()),
            "count": int(short_mask.sum().item()),
        },
        "long_term": {
            "loss_sum": float(loss[~short_mask].sum().item()),
            "count": int((~short_mask).sum().item()),
...
def main() -> None:
    ...
    if not N_USERS and EXPECTED_N_REVIEWS is not None:
        assert eval_aggregate.review_count == EXPECTED_N_REVIEWS, (
            ...
            f"changed in a way that affects review filtering — this is "
...
def _write_diagnostics(eval_aggregate: EvaluationAggregate, grad_summary: dict | None = None, compute_seconds: float = 0.0) -> None:
    ...
    def _flatten(bucket_dict: dict[str, dict[str, float]], names: tuple[str, ...]) -> dict:
        ...
        for name in names:
            b = bucket_dict[name]
            count = int(b["count"])
            out[name] = (b["loss_sum"] / count) if count > 0 else float("nan")
    ...
    diag = build_diagnostics_dict(
        avg_log_loss=eval_aggregate.logloss_by_review,
        logloss_by_user=eval_aggregate.logloss_by_user,
        rows=rows,
    ...
    (result_dir / "diagnostics.json").write_text(json.dumps(diag, indent=2))
    (result_dir / "diagnostics.md").write_text(format_markdown_report(diag))
    print(f"\nWrote diagnostics → {result_dir}/diagnostics.json and .md")
