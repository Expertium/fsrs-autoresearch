...
- **`src/autoresearch/central_diff_init_w.py`** — a meta-optimizer for the model's **default parameters** (note 3 above). It uses Adam with central-difference (black-box) gradients to optimize two distinct sets: the **user-facing default** (applied directly, no per-user training — for someone with little review history) and the **best starting point for per-user SGD** (the init the optimizer descends from). These used to be one tuple; splitting them lets each be optimized for its real job.
